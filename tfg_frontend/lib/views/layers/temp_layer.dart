...
class TempLayer extends StatelessWidget {
  ...
  const TempLayer({super.key, required this.points, required this.camera});
...
class _TempPainter extends CustomPainter {
  ...
  Color _colorForTemp(double t) {
    if (t < -10) return const Color(0xFF4A148C);
    if (t < 0) return const Color(0xFF1A237E);
    if (t < 10) return const Color(0xFF1565C0);
    if (t < 18) return const Color(0xFF00ACC1);
    if (t < 24) return const Color(0xFF43A047);
    if (t < 30) return const Color(0xFFFDD835);
    if (t < 38) return const Color(0xFFFB8C00);
    return const Color(0xFFB71C1C);
  }

  double _opacityForTemp(double t) {
    if (t >= 18 && t <= 24) return 0.25;
    if (t < 0 || t > 35) return 0.55;
    return 0.40;
  }
