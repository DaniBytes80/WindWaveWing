...
class WaveLayer extends StatelessWidget {
  ...
  const WaveLayer({super.key, required this.points, required this.camera});
...
class _WavePainter extends CustomPainter {
  ...
  Color _colorForHeight(double m) {
    if (m < 0.5) return const Color(0xFFB3E5FC);
    if (m < 1.0) return const Color(0xFF4FC3F7);
    if (m < 2.0) return const Color(0xFF0288D1);
    if (m < 3.0) return const Color(0xFF01579B);
    if (m < 4.0) return const Color(0xFF6A1B9A);
